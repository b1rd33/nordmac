...
func TestScopedPlanDerivesOnlyPrivateRoutesAndNoDNS(t *testing.T) {
	plan := scopedTestPlan()
	if err := plan.Validate(); err != nil {
		t.Fatal(err)
	}
	routes := plan.TunnelRoutes()
	if len(routes) != 1 || routes[0].Destination.String() != "10.250.0.0/24" || plan.UsesDNS() {
		t.Fatalf("unexpected scoped policy: routes=%#v DNS=%v", routes, plan.UsesDNS())
	}
	for name, mutate := range map[string]func(*Plan){
		"default route": func(plan *Plan) { plan.ScopedRoutes = []netip.Prefix{netip.MustParsePrefix("0.0.0.0/0")} },
		"public route":  func(plan *Plan) { plan.ScopedRoutes = []netip.Prefix{netip.MustParsePrefix("203.0.113.0/24")} },
		"DNS":           func(plan *Plan) { plan.TunnelDNS = []netip.Addr{netip.MustParseAddr("10.5.0.1")} },
		"duplicate": func(plan *Plan) {
			plan.ScopedRoutes = []netip.Prefix{netip.MustParsePrefix("10.250.0.0/24"), netip.MustParsePrefix("10.250.0.0/24")}
		},
	} {
		t.Run(name, func(t *testing.T) {
			candidate := scopedTestPlan()
			mutate(&candidate)
