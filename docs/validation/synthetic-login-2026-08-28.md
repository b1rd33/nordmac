...
This result proves composition of the reviewed components, not compatibility with Nord's live undocumented endpoint. Bounded hidden/standard-input token collection, a single-writer credential lock, and a fixed production Keychain service were subsequently added and are recorded in the [login-boundary validation](login-boundary-2026-08-28.md). Before enabling the public command, wire those pieces through an authenticated packaged helper, define status/logout and recovery semantics, and obtain separate approval for the exact production request. A real token must never be supplied in argv or an ordinary environment variable.
