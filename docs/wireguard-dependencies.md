...
The current GoReleaser configuration builds only `cmd/nordmac`; it does not ship the harness or link this backend. Before any release begins linking the backend, add the full applicable dependency notices to the release archive and re-run the module, checksum, and license audit. No NordVPN Linux client source is a dependency.
