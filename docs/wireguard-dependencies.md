...
| `golang.org/x/sys` | `v0.47.0` | pinned by `go.sum` | pinned by `go.sum` | BSD-3-Clause |
| `golang.org/x/term` | `v0.45.0` | pinned by `go.sum` | pinned by `go.sum` | BSD-3-Clause |
...
The current `cmd/nordmac` build links the WireGuard backend. Release archives include `THIRD_PARTY_NOTICES.md`; the module graph and architecture-specific linked dependency list must be re-audited before each release. No NordVPN Linux client source is a dependency.
