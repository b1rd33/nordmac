...
1. **Dependency freeze:** review and pin `wireguard-go` and every transitive module by version, checksum, source commit, and license. The current candidate source commit is `ecfc5a8d54462e18e13c72173e2623d16d8e25a0` (2026-05-22), MIT-licensed. No module has been added or downloaded for nordmac yet.
2. **Device-only harness:** in a disposable macOS VM or explicitly approved sacrificial environment, create a userspace `utun` connected only to a controlled WireGuard peer. Configure no default route and no DNS. Prove interface ownership, a fresh handshake, counters, cancellation, process death cleanup, and repeated teardown.
...
Before gate 2, require a new approval naming the reviewed dependency lock, controlled peer endpoint, test environment, maximum duration, and cleanup procedure. An acceptable approval is: “Build the reviewed `wireguard-go` local-peer harness from commit `ecfc5a8d54462e18e13c72173e2623d16d8e25a0` and the recorded module checksums in temporary storage. I approve `sudo`, creation and deletion of the enumerated test `utun` interfaces, synthetic interface addresses, and traffic only to `<controlled endpoint>` for at most `<duration>`. Do not install persistent software, use Nord credentials, add default routes, change DNS or PF, or leave the interfaces running after the test.” The dependency and transitive-license review must be committed before seeking that approval.
