...
The candidate login transaction has also passed end-to-end simulation against one loopback request and an isolated temporary Keychain. It provisioned a synthetic response, stored and verified the token/key pair, deleted both, and removed the Keychain. Bounded hidden/stdin token parsing and a private, fail-closed single-writer credential lock are implemented and tested. This does not enable the public command or validate Nord's live undocumented endpoint. See the [synthetic login record](docs/validation/synthetic-login-2026-08-28.md).
